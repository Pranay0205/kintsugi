...
IMPORTANT RULES FOR GAP IDENTIFICATION:
...
1. For students with Score >= 90%:
   ...
   - These are style preferences, not knowledge gaps for CS1
...
2. For students with Score < 90%:
   ...
   - Efficiency issues are secondary

3. NEVER report these as "gaps" for working code:
   - String concatenation with + (acceptable in CS1)
   - Using substring(i, i+1) instead of charAt(i)
   - Variable naming choices
   - Code that works but is "not optimal"

4. A "knowledge gap" means: Student DOES NOT UNDERSTAND a concept
...
def get_focused_best_attempts(
    problem_ids: list[int] = FOCUS_PROBLEMS,
    max_score: float = 0.9,
    min_students: int = 50
) -> list[dict]:
    """
    Load data, get best attempts, filter to:
    - Specific problems
    - Students with Score < max_score (struggling students)

    Args:
        problem_ids: List of problem IDs to focus on
        max_score: Only include students scoring below this (default 0.9 = 90%)
        min_students: Minimum number of students to return
    """
    ...
    # Filter to struggling students (Score < max_score)
    struggling_df = focused_df[focused_df['Score'] < max_score]

    print(f"\n📊 Filtering Results:")
    print(f"  Total best attempts: {len(best_df):,}")
    print(f"  Focused on problems {problem_ids}: {len(focused_df):,}")
    print(
        f"  Struggling students (Score < {max_score*100:.0f}%): {len(struggling_df):,}")
    print(f"  Unique students: {struggling_df['SubjectID'].nunique()}")

    # Score distribution
    print(f"\n📈 Score Distribution of Struggling Students:")
    bins = [(0, 0.25), (0.25, 0.5), (0.5, 0.75), (0.75, 0.9)]
    for low, high in bins:
        count = len(struggling_df[(struggling_df['Score'] >= low) & (
            struggling_df['Score'] < high)])
        print(f"  {low*100:.0f}%-{high*100:.0f}%: {count} submissions")

    # If not enough struggling students, warn
    if len(struggling_df) < min_students:
        print(
            f"\n⚠️ Warning: Only {len(struggling_df)} struggling submissions found.")
        print(f"   Consider adding more problems or raising max_score threshold.")

    # Convert to list of dicts
    submissions = struggling_df.to_dict('records')
...
def run_analysis(
    ...
    """
    Run the full analysis pipeline on struggling students.

    Args:
        problem_ids: List of problem IDs to focus on
        max_students: Maximum number of students to analyze
        max_score: Only analyze students scoring below this (default 90%)
    """
    print(f"Starting analysis for problems: {problem_ids}")
    print(f"Max students: {max_students}")
    print(f"Focusing on students with Score < {max_score*100:.0f}%")

    # Get focused best attempts (struggling students only)
